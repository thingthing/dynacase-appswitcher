 ...
 *
 * @return boolean return true if need, false otherwise
 *
 */
...
function app_switcher(Action & $action)
{
    ...
    $action->parent->AddJsRef("DCP_APP_SWITCHER:app_switcher.js", true);
    $action->parent->AddJsRef("FDC:setparamu.js", true);
    $action->parent->AddCssRef("DCP_APP_SWITCHER:app_switcher.css", true);
    ...
    if ($dapp == "CORE") $query->basic_elem->sup_where[] = "displayable='Y' or name='CORE'";
    else $query->basic_elem->sup_where[] = "displayable='Y'";
    $list = $query->Query(0, 0, "TABLE");
    $tab = array();
    if ($query->nb > 0) {
        $i = 0;
